...
<html>
	...
	<body>
		...
		<table align='center' width='50%'>
			...
			<?php
				if(!isset($_GET['n']) && !isset($_GET['stu'])){
					$sel_adaptation=mysqli_query($conn,"SELECT * from 
														(SELECT *,max(`write_time`) as `max`,count(`student_id`) as `n` FROM `adaptation_scale_w`
														where `student_id`<>\"\"
														GROUP BY `student_id`
					...
					$sel_adaptation=mysqli_query($conn,"SELECT * from 
														...
														GROUP BY `student_id`
														ORDER BY `student_id`)as `sel1`
														WHERE `n`>1 LIMIT "$n",10");
				}else if(!isset($_GET['n']) && isset($_GET['stu'])){
					$sel_adaptation=mysqli_query($conn,"SELECT * from 
														(SELECT *,max(`write_time`) as `max`,count(`student_id`) as `n` FROM `adaptation_scale_w`
														where `student_id`<>\"\" and `student_id`='".$_GET['stu']."'
														GROUP BY `student_id`
														ORDER BY `student_id`)as `sel1`
														WHERE `n`>1 LIMIT 0,10");
				}else if(isset($_GET['n']) && isset($_GET['stu'])){
					$n=($_GET['n']*10)-10;
					$sel_adaptation=mysqli_query($conn,"SELECT * from 
														(SELECT *,max(`write_time`) as `max`,count(`student_id`) as `n` FROM `adaptation_scale_w`
		...
		<?php
			if(isset($_GET['stu'])){
				$sel_number=mysqli_query($conn,"SELECT * from 
												(SELECT *,max(`write_time`) as `max`,count(`student_id`) as `n` FROM `adaptation_scale_w`
												where `student_id`<>\"\" and `student_id`='".$_GET['stu']."'
												GROUP BY `student_id`
												ORDER BY `student_id`)as `sel1`
